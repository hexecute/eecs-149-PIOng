...
: trace_scanning(false),
  ...
  error_flag(false)
{
	transducer::trace_data_ptr = &data;
}
...
void STLDriver::print() const {

	cout << "\nAssigned formulas:" << endl;
		cout << "-------------------" << endl;

		for (auto formula = formula_map.begin(); formula != formula_map.end(); formula++){
			cout << formula->first << ":" << endl;
	...
	for (auto it= trace_test_queue.begin(); it!= trace_test_queue.end(); it++) {
		...
		cout << (*it).id << ": ";
		cout << (*it).env <<", ";
		cout << "SimTime: " << (*it).sim_time << " Visu:" << (*it).visu << endl;
		for (auto its=  (*it).tests.begin(); its!= (*it).tests.end(); its++) {
			transducer::param_map =  param_map;
			cout << indent << its->test_id << endl;
			for (auto elem = its->param_map.begin(); elem != its->param_map.end(); elem++)
				cout << indent << indent << elem->first << "=" <<  elem->second << endl;

			cout << indent << indent <<  *(*its).formula << endl;
		}
		cout << endl;
	}
	cout << endl << endl;
...
double STLDriver::test_formula(const string & phi_in) {
	...
	string funky_name = "f_u_n_k_y_p_h__i_n_a_m_e";
