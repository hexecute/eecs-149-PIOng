...
#ifdef DEBUG__
	cout << "Entering computeGlobally" << endl;
#endif
...
#ifdef DEBUG__
	cout << "Entering compute_globally" << endl;
#endif
...
//TODO  this routine is unconvential but safe memory-wise (no new/delete) so fix not urgent
void computePartialEventually(Signal* z, Signal::const_reverse_iterator & i, double s,
		double t) {
	bool continued = false;
	double z_max = BOTTOM;
	while (i->time > s) {
